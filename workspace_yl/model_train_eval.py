...
def get_model(gpu=None, arch=""):
  model = models.resnet50(pretrained=True)
  tops = Model_top()
  model = nn.Sequential(model, tops)
...
def main():
  ...
  args.lr = 0.001 * (args.batch_size / 256)
  ...
  model = get_model(gpu=args.gpu, arch=args.arch)
...
def split_adv_examples(dir_path):
  time_str = datetime.now().ctime()
  new_dir_path = './created_adv_false_samples_'+ time_str
  bird_path = new_dir_path + '/bird'
  bicycle_path = new_dir_path + '/bicycle'
  if os.path.isdir(new_dir_path):
    shutil.rmtree(new_dir_path)
  os.mkdir(new_dir_path)
  os.mkdir(bird_path)
  os.mkdir(bicycle_path)
  file_list = glob.glob(dir_path + '/*.png')
  for file_name in file_list:
    label_idx = int(file_name.split('/')[-1].split('_')[1])
    if label_idx == 0:
      shutil.copyfile(file_name, bicycle_path + '/' + file_name.split('/')[-1])
    elif label_idx == 1:
      shutil.copyfile(file_name, bird_path + '/' + file_name.split('/')[-1])
    else:
      raise ValueError('idx parsing error')
